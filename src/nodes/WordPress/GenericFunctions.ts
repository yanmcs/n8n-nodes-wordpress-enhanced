...
export async function execute(this: IExecuteFunctions) {
	...
	const returnData: WordPressResponse[] = [];
		...
		try {
			...
			if (resource === 'post') {
				if (operation === 'getAll') {
					...
					const content = this.getNodeParameter('content', i, '') as string; // Default to empty string if not provided
					...
					const title = this.getNodeParameter('title', i) as string;
					const content = this.getNodeParameter('content', i, undefined) as string | undefined; // Only include if provided

					const body: { title?: string; content?: string } = {};
					if (title) body.title = title;
					if (content !== undefined) body.content = content; // Allow sending empty content
